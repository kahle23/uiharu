package uiharu.persistence.mapper;

import uiharu.persistence.entity.EventRecord;
import org.apache.ibatis.annotations.Mapper;
import org.apache.ibatis.annotations.Param;

import java.util.List;

/**
 * EventRecordMapper.
 * @author Kahle
 * @date 2020-05-24T22:44:07.754+0800
 */
@Mapper
public interface EventRecordMapper {

    /* (Start) This will be covered, please do not modify. */
    /**
     * Insert.
     * @param record The object to insert
     * @return Number of rows effected
     */
    int insert(EventRecord record);

    /**
     * Insert batch.
     * @param recordList A list of records to insert
     * @return Number of rows effected
     */
    int insertBatch(@Param("recordList") List<EventRecord> recordList);

    /**
     * Insert selective.
     * @param record The object to insert
     * @return Number of rows effected
     */
    int insertSelective(EventRecord record);

    /**
     * Delete by primary key.
     * @param id Primary key
     * @param updateUser Current operator
     * @return Number of rows effected
     */
    int deleteByPrimaryKey(@Param("id") String id, @Param("updateUser") String updateUser);

    /**
     * Delete by primary key list.
     * @param idList Primary key list
     * @param updateUser Current operator
     * @return Number of rows effected
     */
    int deleteByPrimaryKeyList(@Param("idList") List<String> idList, @Param("updateUser") String updateUser);

    /**
     * Delete selective.
     * @param record Delete conditions
     * @return Number of rows effected
     */
    int deleteSelective(EventRecord record);

    /**
     * Update by primary key.
     * @param record Content to be updated
     * @return Number of rows effected
     */
    int updateByPrimaryKey(EventRecord record);

    /**
     * Update by primary key selective.
     * @param record Content to be updated
     * @return Number of rows effected
     */
    int updateByPrimaryKeySelective(EventRecord record);

    /**
     * Conditional counting.
     * @param record Query condition
     * @return Count result
     */
    int countSelective(EventRecord record);

    /**
     * Query by primary key.
     * @param id Primary key
     * @return Query result
     */
    EventRecord queryByPrimaryKey(@Param("id") String id);

    /**
     * Query by primary key list.
     * @param idList Primary key list
     * @return Query result list
     */
    List<EventRecord> queryByPrimaryKeyList(@Param("idList") List<String> idList);

    /**
     * Find one.
     * @param record Query condition
     * @return Query result
     */
    EventRecord findOne(EventRecord record);

    /**
     * Query selective.
     * @param record Query condition
     * @return Query result list
     */
    List<EventRecord> querySelective(EventRecord record);
    /* (End) This will be covered, please do not modify. */
    /* Generated by artoria-extend in 2020-05-24T22:44:08.084+0800. */

}
